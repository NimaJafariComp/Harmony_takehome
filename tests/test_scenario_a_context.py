...
def test_seeded_context_selects_current_supplier_update_and_authorized_facts(
    ...
    command = (
        ...
        "detection = StockoutDetector(PostgresErpAdapter(database_url), PostgresAttentionAdapter(database_url), PostgresDemoClock(database_url)).detect(actor, RunId('run-seeded-context'))[0]\n"
