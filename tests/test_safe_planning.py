...
def test_seeded_scenario_a_creates_only_one_pending_approval_and_no_erp_writes(
    ...
    command = (
        ...
        "detector = StockoutDetector(erp, attention_store)\n"
        "first = detector.detect(actor, RunId('run-safe-planning-1'), now)\n"
        "duplicate = detector.detect(actor, RunId('run-safe-planning-2'), now)\n"
