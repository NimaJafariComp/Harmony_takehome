...
def test_seeded_scenario_a_survives_to_tuesday_and_reconstructs_its_complete_audit_run(
    ...
    command = (
        ...
        "assert arrival_tasks[0].payload['audit_run_id'] == str(run_id)\n"
        "arrival = TuesdayArrivalCheckService(erp=erp, identity=identity, attention=attention, audit=audit)\n"
        "arrival_result = arrival.handle_claimed_task(arrival_tasks[0], checked_at=tuesday)\n"
        ...
        "assert 'Rerouted approval to backup approver' in rendered\n"
