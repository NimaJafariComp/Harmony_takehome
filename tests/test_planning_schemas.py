...
def test_seeded_authorized_context_and_candidates_support_a_valid_fake_recommendation(
    ...
    command = (
        ...
        "detection = StockoutDetector(PostgresErpAdapter(database_url), PostgresAttentionAdapter(database_url)).detect(actor, RunId('run-seeded-planning'), datetime(2026, 8, 24, 9, tzinfo=UTC))[0]\n"
