...
def test_seeded_detector_emits_one_stockout_attention_from_scoped_erp_evidence(
    ...
    command = (
        ...
        "detections = StockoutDetector(PostgresErpAdapter(database_url), PostgresAttentionAdapter(database_url), PostgresDemoClock(database_url)).detect(actor, RunId('run-seeded-stockout'))\n"
        "assert len(detections) == 1\n"
