...
def executor_setup(
    *,
    ...
    stored_snapshot = snapshot or WorkflowStateService(store).stage(
        plan,
        created_at=NOW,
...
def test_executor_rejects_tampered_workflow_state_before_claiming_it() -> None:
    ...
    staged = WorkflowStateService(staging_store).stage(plan, created_at=NOW, workflow_id=WORKFLOW_ID)
