...
| Current task | M4.5 - Implement external-style tool boundaries |
| Required task progress | 26 / 58 complete |
| Acceptance criteria progress | 4 / 14 satisfied; 7 in progress |
| Last validated commit | `ae7e132` |
| Last completed-task push | `ae7e132` to `origin/main` |
| Blocking issue | None |
...
| M4.5 Implement external-style tool boundaries | `in_progress` | AC-09, AC-11 | TDD contract is being added before implementation. |
...
| AC-08 Fixed Scenario A workflow | `in_progress` | `po_reroute:v1` resolves only to its immutable six-step declaration and a plan-bound durable snapshot persists that exact ordered state. M4.4 revalidates and leases it, then advances only its declared next guard; external effect steps remain M4.5. |
| AC-09 Idempotency, compensation, and recovery | `in_progress` | The M4.1 catalog derives opaque, stable keys from workflow instance, step, declared tool, and canonical typed input, and assigns compensation metadata to every reviewed effect. M4.3 adds atomic persisted state, plan uniqueness, counters, result/error slots, and leases; M4.4 adds atomic claim/guard transitions. External execution, compensation, and recovery remain M4. |
