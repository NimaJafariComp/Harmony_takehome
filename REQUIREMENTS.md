...
| Use a real LLM API with safe provider boundaries | OpenAI, Claude, and OpenRouter have shared mocked contracts. The recorded current OpenAI Terra and Claude Sonnet 5 full evaluations in [TRANSCRIPT.md](TRANSCRIPT.md) each passed 13/13 synthetic cases and 51/51 checks with no database or business write. The configured OpenRouter free account was rate-limited during the current recheck; its normalized unavailable result exposed no fallback, raw error, credential, or business data. |
